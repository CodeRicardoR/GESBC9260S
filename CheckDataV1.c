...
//Para control de GPIO
	unsigned int pin_control = PC24;
	AT91S_PIO *pioc;
	//------------------GPIO----------------
	pioc = pio_map(PIOC_BASE);
	pio_enable(pioc, pin_control);
	pio_disable_irq(pioc, pin_control);
	pio_disable_multiple_driver(pioc, pin_control);
	pio_disable_pull_ups(pioc, pin_control);
	pio_synchronous_data_output(pioc, pin_control);
	pio_output_enable(pioc, pin_control);
	pio_out(pioc, pin_control, 0);
	//--------------------------------------
	...
	if (num){
		...
		if(f_status == NULL){
			...
			if (reset_system == 0){
				...
				//-----------------------------------------------------------------
				//Control de GPIO
				pio_out(pioc, pin_control, 1);
				usleep(10000000);	
				pio_out(pioc, pin_control, 0);
				//Reset de sistema embebido
				//-----------------------------------------------------------------
				f_cmd = popen("reboot","r");
				if (f_cmd == NULL){
					f_log = fopen(filelog,"a");
					if (f_log == NULL)
						printf("No se puedo abrir statuslog.txt\n");
